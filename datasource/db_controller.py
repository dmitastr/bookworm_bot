...
from common.config import DB_MAPPING, REPORTS_TABLE
...
class YDataBase:
    def __init__(self, db_name: str='greetings') -> None:
        DB_CONFIG = DB_MAPPING.get(db_name)
        if DB_CONFIG:
            self.endpoint = os.getenv(DB_CONFIG["ENDPOINT"])
            self.database = os.getenv(DB_CONFIG["DATABASE"])
        else:
...
def reports_to_user(user_id: int, chat_id: int, cursor: YDataBase) -> list[dict]:
    reports = cursor.get_fields_equal(
        field_filter=dict(suspected_user_id=[user_id], chat_id=[chat_id]), 
